...
```{r plot4b, echo=FALSE,message=FALSE}
subset(dat,attack=="hanau" & full_days_since_attack>0) %>% group_by(journal,full_days_since_attack) %>% summarize(nothing=sum(count)==0) %>% filter(nothing) %>%
    ...
    labs(x="Zeitung",y="",title="Tage ohne überhaupt eine Veröffentlichung zum Anschlag in Hanau\n(im letzten Jahr)")+ylim(0,365)+
  theme_simon+ scale_fill_manual(values = c("sienna1", "sienna4", "tomato2", "hotpink4"))+theme(legend.position = "none")
```
...
<hr>
Zum Berliner Attentat gibt es im ersten Jahr über `r conservative(nrow(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365  & count>0)),2)`  Suchergebnisse - fast jeden zweiten Tag findet sich in jeder der Zeitungen eine Erwähnung. Bei Bild noch öfter.<br/>
Bild: `r nrow(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:366 & count>0 & journal=="bild"))` Such-Hits (`r nrow(unique(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365  & count>0 & journal=="bild",select="full_days_since_attack")))` Tage)<br/>
SZ: `r nrow(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365& full_days_since_attack>=0 & count>0 & journal=="sz"))` Hits (`r nrow(unique(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365  & count>0 & journal=="sz",select="full_days_since_attack")))` Tage)<br/>
FAZ: `r nrow(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365& full_days_since_attack>=0 & count>0 & journal=="faz"))` Hits (`r nrow(unique(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365  & count>0 & journal=="faz",select="full_days_since_attack")))` Tage)<br/>
Spiegel: `r nrow(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365 & full_days_since_attack>=0 & count>0 & journal=="spiegel"))` Hits  (`r nrow(unique(subset(dat,attack=="berlin" & full_days_since_attack %in% 0:365  & count>0 & journal=="spiegel",select="full_days_since_attack")))` Tage)
...
ggplot(aes(x=journal,fill=attack)) +
    labs(x="Zeitung",title="Tage im ersten Jahr ohne überhaupt eine Veröffentlichung zum Thema",y="")+
    theme_simon+scale_fill_colorblind()+
...
<hr>
Der Code und weitere erklärende Details sind zugänglich, prüfbar und replizierbar unter: github.com/simonheb/hanau-media.<br/>
Die Daten sind via: simonheb.shinyapps.io/hanau-media/ einsehbar.<br/>
Es gibt natürlich keine Garantie für die Richtigkeit der Zahlen und Ergänzungen und Korrekturen sind willkommen<br/>
